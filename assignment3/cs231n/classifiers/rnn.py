...
class CaptioningRNN(object):
    ...
    def sample(self, features, max_length=30):
        ...
        N, D = features.shape
        affine_out, affine_cache = affine_forward(features, W_proj, b_proj)
        caption_in = [self._start] * N
        h = affine_out
        c = np.zeros(h.shape)
        for t in range(max_length):
            if self.cell_type == 'rnn':
                h, _ = rnn_step_forward(W_embed[caption_in], h, Wx, Wh, b)
            else:
                h, c, _ = lstm_step_forward(W_embed[caption_in], h, c, Wx, Wh, b)
            vocab_scores, _ = affine_forward(h, W_vocab, b_vocab)
            # vocab_scores = affine_forward(h, W_vocab, b_vocab) 漏写_会导致score读到的是（out,cache） 导致后面出错
            pred = np.argmax(vocab_scores, axis=1)
            captions[:,t] = pred
            caption_in = pred
        ############################################################################
        #                             END OF YOUR CODE                             #
