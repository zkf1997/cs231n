...
def rnn_backward(dh, cache):
    ...
    for t in range(T):
        ...
        dnext_h = dh[:, t, :] + dprev_h#dloss/dht + dloss/dht+1 * dht+1/dht  得到的dh不是真正的导数，计算图里每个h到loss有一条线，到ht+1也有一条线，输入的dh只包括了到loss那条线
        dx[:, t, :], dprev_h, dWxt, dWht, dbt = rnn_step_backward(dnext_h, step_cache)
        dWx, dWh, db = dWx + dWxt, dWh + dWht, db + dbt#dw是对各时间步的合
        next_h = prev_h
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (i, f, o, g, prev_c, prev_h, next_c, Wx, Wh, x)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    N, H = dnext_h.shape
    i, f, o, g, prev_c, prev_h, next_c, Wx, Wh, x = cache
    _, D = x.shape
    t = np.tanh(next_c)
    do = dnext_h * t
    dnext_c += dnext_h * o * (1 - t * t)#同样的，dnextc不是真正的dnextc，h也是c的函数，dloss/dnextc = dnextc + dloss/dnexth * dnexth/dnextc
    df = dnext_c * prev_c
    dprev_c = dnext_c * f
    di = dnext_c * g
    dg = dnext_c * i
...
def lstm_backward(dh, cache):
    ...
    #############################################################################
    N, T, H = dh.shape
    z_i, z_f, z_o, z_g, prev_c, prev_h, next_c, Wx, Wh, x = cache[T - 1]
    D = x.shape[1]
    ...
    for t in range(T):
        t = T - 1 - t
        step_cache = cache[t]
        dnext_h = dh[:, t, :] + dprev_h#同理 梯度要加上dloss/dht+1 * dht+1/dht
        dnext_c = dprev_c#c在这里不加上dloss/dh * dh/dc，在lstm_step里面加上运算更方便，而dnexth因为需要dprevh，所以不方便再step里面加上
        # if t == T - 1:
        #     print(dnext_c)
